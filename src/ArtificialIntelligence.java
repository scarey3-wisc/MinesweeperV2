...
public class ArtificialIntelligence extends Player{
		...
		while(true){
			...
			if(good.size() > 0){
				...
				if(counts.size() == 1 || good.size() == 0 && partitionList.get(0).size() > 0){
					LinkedPoint randomlyChosen = partitionList.get(0).get((int) (Math.random() *
							partitionList.get(0).size()));
					...
					if(i < partitionList.size()) {
						LinkedPoint randomlyChosen = partitionList.get(i).get((int) (Math.random() *
								partitionList.get(i).size()));
						System.out.println("Guess");
						myGame.clickSpot(randomlyChosen.x, randomlyChosen.y);
	...
	/*
	 * partition: a list of unknown cells that we're exploring
	 * theory: an array, where 0 means 'no bomb', 1 means 'bomb', and -1 means 'idk'
	 * counter: records how many bombs have appeared at a particular index in valid hypothesis
	 * in each of these three arrays, the same index refers to the same point - except that counter
	 * has an extra element for the total number of valid hypothesis.
	 * 
	 * order: a list of indexes; the order in which to build theories
	 * depth: which index in the order to do next
	 */
	private int testHypothesis(ArrayList<LinkedPoint> partition, 
			int[] theory, ArrayList<Long> counter, ArrayList<Integer> order, int depth){
		...
		//Step five: if we haven't reached the bottom, recurse with a bomb in and not in
		//the next slot.
